...
def aso(High, Low, Open,Close, length,**kwargs):
    ...
    data["direction"]=data.apply(lambda row: 0 if row['ASOBulls'] < row['ASOBears'] else 1, axis=1)
...
def BCL2ECTS(High, Low, Open,Close, fast_length,slow_length,buy_threshold,sell_threshold,**kwargs):
    ...
    return data.Signals
                    ...
                    ######################### THE PRICE RADIO ################  das ist fertig
...
def TPR(High, Low, Open,Close,length,**kwargs):
    ...
    data["AM+"].fillna(data["AM+"][16],inplace=True)
    data["AM-"].fillna(data["AM-"][16],inplace=True)
    data.FM.fillna(data.FM[13],inplace=True)

    data['Signals'] = data['FM'].apply(lambda x: 1 if x > 0 else 0)
